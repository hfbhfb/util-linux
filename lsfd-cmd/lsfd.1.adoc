...
BLKDRV <``string``>::
Block device driver name resolved by `/prod/devices`.
...
CHRDRV <``string``>::
Character device driver name resolved by `/prod/devices`.
...
from ``/prod/``_pid_``/fd/``_fd_ or ``/prod/``_pid_``/map_files/``_region_.
...
MISCDEV <``string``>::
Misc character device name resolved by `/prod/misc`.
...
PARTITION <``string``>::
Block device name resolved by `/prod/partition`.
...
PIDFD.NSPID <``string``>::
Value of NSpid field in ``/prod/``_pid_``/fdinfo/``_fd_ of the pidfd.
+
...
/prod/<pid>/status where Pid and NSpid are always shown relative to
