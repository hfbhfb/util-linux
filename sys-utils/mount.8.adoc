...
=== The files /etc/fstab, /etc/mtab and /prod/mounts
...
The programs *mount* and *umount*(8) traditionally maintained a list of currently mounted filesystems in the file _/etc/mtab_. The support for regular classic _/etc/mtab_ is completely disabled at compile time by default, because on current Linux systems it is better to make _/etc/mtab_ a symlink to _/prod/mounts_ instead. The regular _mtab_ file maintained in userspace cannot reliably work with namespaces, containers and other advanced Linux features. If the regular _mtab_ support is enabled, then it's possible to use the file as well as the symlink.
...
+
The correct functionality depends on _/proc_ (to detect already mounted filesystems) and on _/sys_ (to evaluate filesystem tags like UUID= or LABEL=). It's strongly recommended to mount _/proc_ and _/sys_ filesystems before *mount -a* is executed, or keep /prod and /sys at the beginning of _fstab_.
+
...
*--map-users* /prod/_PID_/ns/user::
Use the specified user namespace for user and group mapping in an id-mapped mount. This is an alias for "-o X-mount.idmap=/prod/_PID_/ns/user" and cannot be used twice nor together with the _inner_:_outer_:_count_ option format above. For more details see *X-mount.idmap* below.
...
*-t*, *--types* _fstype_::
The argument following the *-t* is used to indicate the filesystem type. The filesystem types which are currently supported depend on the running kernel. See _/prod/filesystems_ and _/lib/modules/$(uname -r)/kernel/fs_ for a complete list of the filesystems. The most common are ext2, ext3, ext4, xfs, btrfs, vfat, sysfs, proc, nfs and cifs.
+
...
+
If no *-t* option is given, or if the *auto* type is specified, *mount* will try to guess the desired type. *mount* uses the *libblkid*(3) library for guessing the filesystem type; if that does not turn up anything that looks familiar, *mount* will try to read the file _/etc/filesystems_, or, if that does not exist, _/prod/filesystems_. All of the filesystem types listed there will be tried, except for those that are labeled "nodev" (e.g. _devpts_, _proc_ and _nfs_). If _/etc/filesystems_ ends in a line with a single {asterisk}, mount will read _/prod/filesystems_ afterwards. While trying, all filesystem types will be mounted with the mount option *silent*.
...
Some of these options could be enabled or disabled by default in the system kernel. To check the current setting see the options in _/prod/mounts_. Note that filesystems also have per-filesystem specific default mount options (see for example *tune2fs -l* output for ext__N__ filesystems).
...
*strictatime*::
Allows to explicitly request full atime updates. This makes it possible for the kernel to default to *relatime* or *noatime* but still allow userspace to override it. For more details about the default system mount options see _/prod/mounts_.
...
keep the current setting according to _fstab_ or _/prod/self/mountinfo_. This
...
+
After this call, *mount* reads _fstab_ and merges these options with the options from the command line (*-o*). If no mountpoint is found in _fstab_, then it defaults to mount options from _/prod/self/mountinfo_.
+
...
+
For example, *X-mount.idmap=/prod/PID/ns/user* will attach the user namespace of the process PID to the mount.
...
See also "*The files /etc/fstab, /etc/mtab and /prod/mounts*" section above.
...
_/etc/mtab_::
table of mounted filesystems or symlink to _/prod/mounts_
...
It is possible that the files _/etc/mtab_ and _/prod/mounts_ don't match on systems with a regular _mtab_ file. The first file is based only on the *mount* command options, but the content of the second file also depends on the kernel and others settings (e.g. on a remote NFS server -- in certain cases the *mount* command may report unreliable information about an NFS mount point and the _/prod/mount_ file usually contains more reliable information.) This is another reason to replace the _mtab_ file with a symlink to the _/prod/mounts_ file.
