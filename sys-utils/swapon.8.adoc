...
*swapoff* disables swapping on the specified devices and files. When the *-a* flag is given, swapping is disabled on all known swap devices and files (as found in _/prod/swaps_ or _/etc/fstab_).
...
*-L* _label_::
Use the partition that has the specified _label_. (For this, access to _/prod/partitions_ is needed.)
...
*-s*, *--summary*::
Display swap usage summary by device. Equivalent to *cat /prod/swaps*. This output format is DEPRECATED in favour of *--show* that provides better control on output data.
