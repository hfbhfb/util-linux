...
+
If no devices are given, the devices mentioned in _/prod/partitions_ (if this file exists) are used. Devices are always listed in the order in which they are specified on the command-line, or by the kernel listed in _/prod/partitions_.
